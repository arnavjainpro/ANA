...
export function IndexingProgress(): JSX.Element | null {
  ...
  return (
    ...
    >
      <div className="h-1 w-full overflow-hidden bg-surface-overlay">
        <div
          className="ana-progress-fill h-full bg-accent-primary"
          style={{ width: `${pct}%` }}
        />
      </div>
      <p className="bg-surface-raised px-6 py-1 text-xs text-text-secondary" aria-live="polite">
        {indexMessage}
      </p>
    </div>
  );
